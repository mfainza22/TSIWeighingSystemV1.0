    ...
    Private Sub ListData()
            ...
            With cls.DataReader
                If .HasRows Then
                    ...
                    While .Read
                        ...
                        Rw.Cells(Col_BKID.Name).Value = .Item("PlateNo")
                        If IsDBNull(.Item("Prev_tareWt")) Then Rw.Cells(Col_Tare.Name).Value = "" Else Rw.Cells(Col_Tare.Name).Value = Trim(.Item("Prev_tareWt").ToString)
                        If IsDBNull(.Item("DriverName")) Then Rw.Cells(Col_Driver.Name).Value = "" Else Rw.Cells(Col_Driver.Name).Value = Trim(.Item("DriverName").ToString)
                        If IsDBNull(.Item("PActive")) Then Rw.Cells(Col_active.Name).Value = False Else Rw.Cells(Col_active.Name).Value = CBool(Trim(.Item("PActive").ToString))
        ...
        Catch ex As Exception
            ...
            With CL
                ...
                .LogLocation = "Truck Maintenance Window"
                .CreateErrorLog()
            End With
...
#Region "CELL VALIDATING AND SAVING"
    ...
    Private Sub Dg_CellBeginEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles Dg.CellBeginEdit
        ...
        Else
            EditSwitch = True
            PreviousVal = Dg.CurrentRow.Cells(0).Value
        End If
        BeginEdit = True
    ...
    Private Sub Dg_RowValidating(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles Dg.RowValidating
        ...
        If Dg.Rows(e.RowIndex).Cells(Col_BKID.Name).Value = Nothing Then
            AddSwitch = True
            EditSwitch = False
        Else
            AddSwitch = False
        ...
        Cellvalue = Dg.Rows(e.RowIndex).Cells(0).Value
        If String.IsNullOrEmpty(Cellvalue) Then
            ErrorText = ErrorText & Environment.NewLine & "Plate NUmber is required."
        Else
            Select Case True
                Case AddSwitch
                    If MOD_DATABASEPROC.ExistenceFound("Select * from Vehicle_tbl where PlateNo = '" & Trim(Cellvalue) & "'", Nothing, Nothing, Nothing) Then
                        ErrorText = ErrorText & Environment.NewLine & "Plate Number is already in the database."
                    End If
                Case EditSwitch
                    If MOD_DATABASEPROC.ExistenceFound("Select * from Vehicle_tbl where PlateNo ='" & Trim(Dg.Rows(e.RowIndex).Cells(Col_BKID.Name).Value) & "'", Nothing, "PlateNo", Nothing) Then
                        If ReturnedData.Equals(Dg.Rows(e.RowIndex).Cells(Col_BKID.Name).Value) = False Then
        ...
        Dim Tare As String = _
Replace(Dg.Rows(e.RowIndex).Cells(Col_Tare.Name).Value, ",", "")
        If String.IsNullOrEmpty(Tare) Then Tare = 0
        If IsNumeric(Tare) = False Then
            ErrorText = ErrorText & Environment.NewLine & "Tare should be numeric."
        End If
        ...
        If e.Cancel = False Then
            ...
            If AddSwitch = True Then
                If Dg.CurrentRow.ErrorText = Nothing Then
                    ' Dim Cls_Crypt As New CLS_CRYPTO
                    With Saver
                        Saver.Table = "Vehicle_Tbl"
                        Saver.Source = "Insert into " & Saver.Table & " (PlateNo,Prev_tareWt,DriverName,Pactive) values (" & _
                             "'" & Trim(Dg.Rows(e.RowIndex).Cells(Col_PlateNo.Name).Value) & "'" & _
                                ",'" & Tare & "'" & _
                                 ",'" & Trim(Dg.Rows(e.RowIndex).Cells(Col_Driver.Name).Value) & "'" & _
                                   "," & Active & ")"
                    ...
                    '//////////////// RECOVER ID
                    'If ExistenceFound("Select * from Vehicle_Tbl where PlateNO = '" & Trim(Dg.Rows(e.RowIndex).Cells(Col_PlateNo.Name).Value) & "'", Nothing, "DriverId", Nothing) = True Then
                    Dg.Rows(e.RowIndex).Cells(Col_BKID.Name).Value = Dg.Rows(e.RowIndex).Cells(Col_PlateNo.Name).Value
                    'End If

                    TxtSearch.AutoCompleteCustomSource.Add(Trim(Dg.Rows(e.RowIndex).Cells(0).Value))
            ...
            If EditSwitch = True Then
                If Dg.CurrentRow.ErrorText = Nothing Then
                    Saver.Table = "Vehicle_Tbl"
                    Saver.Source = "Update " & Saver.Table & " Set PlateNo = '" & Dg.Rows(e.RowIndex).Cells(Col_PlateNo.Name).Value.trim & "'" & _
                        ",Prev_TareWt= '" & Tare & "'" & _
                        ",DriverName = '" & Dg.Rows(e.RowIndex).Cells(Col_Driver.Name).Value.trim & "'" & _
                         ",PActive =" & Active & "" & _
                              " where PlateNo= '" & Dg.CurrentRow.Cells(Col_PlateNo.Name).Value.trim & "'"
    ...
    Private Sub Dg_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles Dg.CellFormatting
        If e.ColumnIndex.Equals(Col_Tare.Index) = False Then Exit Sub
        If EnableValidation = True Then Exit Sub

        Dim Tare As String = Replace(Dg.Rows(e.RowIndex).Cells(e.ColumnIndex).Value, ",", "")
        If String.IsNullOrEmpty(Tare) = True Then
            Dg.Rows(e.RowIndex).Cells(e.ColumnIndex).Value = "0"
        Else
            If IsNumeric(Tare) = False Then Exit Sub
            Dg.Rows(e.RowIndex).Cells(e.ColumnIndex).Value = FormatNumber(Tare, 0, TriState.True, TriState.True, TriState.True)
        End If
    End Sub
#End Region
    ...
    Private Sub Dg_UserDeletingRow(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewRowCancelEventArgs) Handles Dg.UserDeletingRow
        Try
            ...
            Else
                ...
                If MessageBox.Show("Are you sure you want to delete selected record\s?", "", MessageBoxButtons.YesNo, _
                    ...
                    End If
                    Remover.Table = "Vehicle_Tbl"
                    Remover.FieldName = "PlateNo"
                    Remover.Text = e.Row.Cells(0).Value
                    Remover.Source = "Delete From " & Remover.Table & " where PlateNo = '" & Remover.Text & "'"
        ...
        Catch ex As Exception
            ...
            With CL
                .User = FrmLogin.UserDisplayName
                .LogError = ex.Message
                .LogLocation = "Trucks Maintenance Window"
            ...
            If e.KeyCode = Keys.Escape Then
                Select Case True
                    ...
                    Case EditSwitch
                        ...
                        With Restoration
                            .source = "Select * from Vehicle_Tbl where PlateNo = " & Trim(Dg.CurrentRow.Cells(0).Value) & ""
                            .opensource()
                        End With
