            ...
            With cls.DataReader
                If .HasRows Then
                    ...
                    While .Read
                        ...
                        If IsDBNull(.Item("SupCode")) Then Rw.Cells(Col_Code.Name).Value = "" Else Rw.Cells(Col_Code.Name).Value = Trim(.Item("SupCode").ToString)
                        If IsDBNull(.Item("SUpName")) Then Rw.Cells(Col_Name.Name).Value = "" Else Rw.Cells(Col_Name.Name).Value = Trim(.Item("Supname").ToString)
                        If IsDBNull(.Item("SupAddr")) Then Rw.Cells(Col_addr.Name).Value = "" Else Rw.Cells(Col_addr.Name).Value = Trim(.Item("SupAddr").ToString)
                        If IsDBNull(.Item("SContactP")) Then Rw.Cells(Col_Contactp.Name).Value = "" Else Rw.Cells(Col_Contactp.Name).Value = Trim(.Item("SContactP"))
                        If IsDBNull(.Item("STelno")) Then Rw.Cells(Col_addr.Name).Value = "" Else Rw.Cells(Col_telno.Name).Value = Trim(.Item("STelNo"))
                        If IsDBNull(.Item("SActive")) Then Rw.Cells(Col_active.Name).Value = False Else Rw.Cells(Col_active.Name).Value = CBool(Trim(.Item("SActive").ToString))
    ...
    Private Sub Dg_RowValidating(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles Dg.RowValidating
            ...
            Else
                Select Case True
                    ...
                    Case EditSwitch
                        ...
                        If MOD_DATABASEPROC.ExistenceFound("select * From Sup_Tbl where SupId = " & Dg.CurrentRow.Cells(0).Value & "", Nothing, "SupCode", Nothing) Then
                            If Trim(ReturnedData) <> Trim(Cellvalue) Then
                                If MOD_DATABASEPROC.ExistenceFound("select * From Outbound_Tbl where SupplierCode = '" & ReturnedData & "'", Nothing, Nothing, Nothing) Then
                                    ErrorText = ErrorText & Environment.NewLine & "* Code cannot cannot be edited because it is being used by another table."
                                End If
                            End If
                        Else
                            If MOD_DATABASEPROC.ExistenceFound("Select * from Sup_tbl where SupCode= '" & Trim(Cellvalue) & "' and SupId <>" & Dg.CurrentRow.Cells(0).Value & "", Nothing, Nothing, Nothing) Then
                                ErrorText = ErrorText & Environment.NewLine & "Specified Code is already in the database."
                            End If
                        End If
                End Select
            ...
            Else
                Select Case True
                    Case AddSwitch
                        ...
                        End If
                    Case EditSwitch
                        If MOD_DATABASEPROC.ExistenceFound("Select * from Sup_tbl where Supname= '" & Trim(Cellvalue) & "' and SupId <>" & Dg.CurrentRow.Cells(0).Value & "", Nothing, Nothing, Nothing) Then
                            ErrorText = ErrorText & Environment.NewLine & "Specified name is already in the database."
                        End If
                End Select
            End If
            ...
            If e.Cancel = False Then
                ...
                If AddSwitch = True Then
                    If Dg.CurrentRow.ErrorText = Nothing Then
                        ...
                        With Saver
                            Saver.Table = "Sup_tbl"
                            Saver.Source = "Insert into " & Saver.Table & " (SupCode,SupName,SupAddr,SContactP,STelno,SActive) values (" & _
                        ...
                        '//////////////// RECOVER ID
                        If ExistenceFound("Select * from Sup_tbl where SupCode = '" & Trim(Dg.Rows(e.RowIndex).Cells(Col_Code.Name).Value) & "'", Nothing, "SupId", Nothing) = True Then
                ...
                If EditSwitch = True Then
                    If Dg.CurrentRow.ErrorText = Nothing Then
                        Saver.Table = "Sup_tbl"
                        Saver.Source = "Update " & Saver.Table & " Set SupCode = '" & Trim(Dg.Rows(e.RowIndex).Cells(Col_Code.Name).Value) & "'" & _
                            ",Supname = '" & Trim(Dg.Rows(e.RowIndex).Cells(Col_Name.Name).Value) & "'" & _
                             ",SupAddr = '" & Trim(Dg.Rows(e.RowIndex).Cells(Col_addr.Name).Value) & "'" & _
                            ",SContactP= '" & Trim(Dg.Rows(e.RowIndex).Cells(Col_Contactp.Name).Value) & "'" & _
                            ",STelno= '" & Trim(Dg.Rows(e.RowIndex).Cells(Col_telno.Name).Value) & "'" & _
                            ",SActive =" & Active & "" & _
                              " where SupId = " & Dg.CurrentRow.Cells(0).Value & ""
        ...
        Catch ex As Exception
            ...
            With CL
                ...
                .LogLocation = "Supplier Maintenance Window"
                .CreateErrorLog()
            End With
            MessageBox.Show(ex.Message, "", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub Dg_UserDeletingRow(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewRowCancelEventArgs) Handles Dg.UserDeletingRow
        Try
            ...
            Else
                ...
                If MOD_DATABASEPROC.ExistenceFound("select * From Outbound_Tbl where SupplierCode = '" & Dg.CurrentRow.Cells(1).Value & "'", Nothing, Nothing, Nothing) Then
            ...
            If MessageBox.Show("Are you sure you want to delete selected record\s?", "", MessageBoxButtons.YesNo, _
                ...
                End If
                Remover.Table = "Sup_tbl"
                Remover.FieldName = "SupCode"
            ...
            If e.KeyCode = Keys.Escape Then
                ...
                Select Case True
                    ...
                    Case EditSwitch
                        ...
                        With Restoration
                            .source = "Select * from Sup_tbl where SupId = " & Trim(Dg.CurrentRow.Cells(Col_Id.Name).Value) & ""
                            .opensource()
                        End With
